  ...
  export default {
    ...
    methods:{
      getParent() {
        ajax({
          method: 'get',
          url: utils.stringFormat("http://{0}:{1}/menu/parent",[this.svr['server_ip'], this.svr['server_port']]),
          timeout: 1000,
        }).then((res) => {
      ...
      queryMenu() {
        ajax({
          method: 'get',
          url: utils.stringFormat("http://{0}:{1}/menu",[this.svr['server_ip'], this.svr['server_port']]),
          params: {
            name  : this.menu_name,
          },
          timeout: 1000,
      ...
      openEdit:function(index,row){
        ...
        ajax({
          method: 'get',
          url: utils.stringFormat("http://{0}:{1}/menu/{2}",[this.svr['server_ip'], this.svr['server_port'],row.Id]),
          timeout: 1000,
        }).then((res) => {
      ...
      delMenu:function(index,row) {
        this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          ajax.delete(utils.stringFormat("http://{0}:{1}/menu",[this.svr['server_ip'], this.svr['server_port']]),
              { params:
                        {
                          id:row.Id
                     }
