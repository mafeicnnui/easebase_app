  ...
  export default {
    ...
    methods:{
      getQx() {
        ajax({
          method: 'get',
          url: utils.stringFormat("http://{0}:{1}/menu/role/qx",[this.svr['server_ip'], this.svr['server_port']]),
          timeout: 1000,
        }).then((res) => {
      ...
      queryRole() {
        ajax({
          method: 'get',
          url: utils.stringFormat("http://{0}:{1}/role",[this.svr['server_ip'], this.svr['server_port']]),
          params: {
            name  : this.role_name,
          },
          timeout: 1000,
      ...
      openEdit:function(index,row){
        ...
        ajax({
          method: 'get',
          url: utils.stringFormat("http://{0}:{1}/role/{2}",[this.svr['server_ip'], this.svr['server_port'],row.Id]),
          timeout: 1000,
        }).then((res) => {
          ...
          if (res.data['Code'] == 200 ) {
            ...
            ajax({
              method: 'get',
              url: utils.stringFormat("http://{0}:{1}/role/privileges/{2}",[this.svr['server_ip'], this.svr['server_port'],res.data['Data'].Id]),
              timeout: 3000,
            }).then((res) => {
      ...
      delRole:function(index,row) {
        ...
        this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          ajax.delete(utils.stringFormat("http://{0}:{1}/role",[this.svr['server_ip'], this.svr['server_port']]),
              { params:
                        {
                          role_id:row.Id
                        }
